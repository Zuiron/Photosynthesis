...
import net.zuiron.photosynthesis.screen.DryingNetScreenHandler;
import org.jetbrains.annotations.Nullable;

import java.util.Optional;

public class DryingNetBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public DryingNetBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.DRYINGNET, pos, state);

        this.propertyDelegate = new PropertyDelegate() {
            public int get(int index) {
                switch (index) {
                    case 0: return DryingNetBlockEntity.this.progress;
                    case 1: return DryingNetBlockEntity.this.maxProgress;
                    default: return 0;
                }
            }
            ...
            public void set(int index, int value) {
                switch(index) {
                    case 0: DryingNetBlockEntity.this.progress = value; break;
                    case 1: DryingNetBlockEntity.this.maxProgress = value; break;
                }
            }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        return new DryingNetScreenHandler(syncId, inv, this, this.propertyDelegate);
    }
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("dryingnet.progress", progress);
        nbt.putInt("dryingnet.cookingTime", maxProgress);
    }
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        super.readNbt(nbt);
        progress = nbt.getInt("dryingnet.progress");
        maxProgress = nbt.getInt("dryingnet.cookingTime");
    }
    ...
    public static void tick(World world, BlockPos blockPos, BlockState state, DryingNetBlockEntity entity) {
        if(world.isClient()) {
            return;
        }
    ...
    private static void craftItem(DryingNetBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<DryingNetRecipe> recipe = entity.getWorld().getRecipeManager()
                .getFirstMatch(DryingNetRecipe.Type.INSTANCE, inventory, entity.getWorld());
    ...
    private static boolean hasRecipe(DryingNetBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<DryingNetRecipe> match = entity.getWorld().getRecipeManager()
                .getFirstMatch(DryingNetRecipe.Type.INSTANCE, inventory, entity.getWorld());

        entity.maxProgress = entity.getWorld().getRecipeManager()
                .getFirstMatch(DryingNetRecipe.Type.INSTANCE, inventory, entity.getWorld()).map(DryingNetRecipe::getCookTime).orElse(20);
    ...
    public boolean canInsert(int slot, ItemStack stack, @Nullable Direction side) {
        Direction localDir = this.getWorld().getBlockState(pos).get(DryingNetBlock.FACING);
    ...
    public boolean canExtract(int slot, ItemStack stack, Direction side) {
        Direction localDir = this.getWorld().getBlockState(this.pos).get(DryingNetBlock.FACING);
