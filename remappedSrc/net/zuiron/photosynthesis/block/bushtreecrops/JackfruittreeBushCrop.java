...
public class JackfruittreeBushCrop extends SweetBerryBushBlock {

    public JackfruittreeBushCrop(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.JACKFRUIT);
    }
    ...
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        if(world.getBlockState(pos.up(2)).isOf(ModBlocks.JACKFRUITTREE_LEAVES) && !world.getBlockState(pos.up(2)).get(Properties.PERSISTENT)) {
            return true;
        } else {
            return false;
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!bl && player.getStackInHand(hand).isOf(Items.BONE_MEAL)) {
            ...
            dropStack(world, pos, new ItemStack(ModItems.JACKFRUIT, j + (bl ? 1 : 0)));
