...
public class GravityPressBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof GravityPressBlockEntity) {
                ItemScatterer.spawn(world, pos, (GravityPressBlockEntity)blockEntity);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos,
        ...
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((GravityPressBlockEntity) world.getBlockEntity(pos));
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GravityPressBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.GRAVITYPRESS, GravityPressBlockEntity::tick);
    }
}
