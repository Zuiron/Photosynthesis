...
public class LimetreeBushCrop extends SweetBerryBushBlock {

    public LimetreeBushCrop(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.LIME);
    }
    ...
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        if(world.getBlockState(pos.up(2)).isOf(ModBlocks.LIMETREE_LEAVES) && !world.getBlockState(pos.up(2)).get(Properties.PERSISTENT)) {
            return true;
        } else {
            return false;
        }
