...
public class PawpawtreeBushCrop extends SweetBerryBushBlock {

    public PawpawtreeBushCrop(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.PAWPAW);
    }
    ...
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        if(world.getBlockState(pos.up(2)).isOf(ModBlocks.PAWPAWTREE_LEAVES) && !world.getBlockState(pos.up(2)).get(Properties.PERSISTENT)) {
            return true;
        } else {
            return false;
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!bl && player.getStackInHand(hand).isOf(Items.BONE_MEAL)) {
            ...
            dropStack(world, pos, new ItemStack(ModItems.PAWPAW, j + (bl ? 1 : 0)));
