...
import net.zuiron.photosynthesis.screen.ManureBucketScreenHandler;
...
public class ManureBucketBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public ManureBucketBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MANUREBUCKET, pos, state);
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        return new ManureBucketScreenHandler(syncId, inv, this);
    }
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("manurebucket.cooldown", cooldown);
    }

    @Override
    public void readNbt(NbtCompound nbt) {
        Inventories.readNbt(nbt, inventory);
        cooldown = nbt.getInt("manurebucket.cooldown");
        super.readNbt(nbt);
    }
    ...
    public static void tick(World world, BlockPos blockPos, BlockState state, ManureBucketBlockEntity entity) {
        if(world.isClient()) {
            return;
        }
        ...
        if(entity.cooldown < 20*2) { //20 ticks per second. 1 item transfer per 2 seconds.
            return;
        }
