...
import net.zuiron.photosynthesis.screen.HayRackScreenHandler;
import org.jetbrains.annotations.Nullable;

import java.util.Optional;

public class HayRackBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public HayRackBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.HAYRACK, pos, state);

        this.propertyDelegate = new PropertyDelegate() {
            public int get(int index) {
                switch (index) {
                    case 0: return HayRackBlockEntity.this.progress;
                    case 1: return HayRackBlockEntity.this.maxProgress;
                    default: return 0;
                }
            }
            ...
            public void set(int index, int value) {
                switch(index) {
                    case 0: HayRackBlockEntity.this.progress = value; break;
                    case 1: HayRackBlockEntity.this.maxProgress = value; break;
                }
            }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        return new HayRackScreenHandler(syncId, inv, this, this.propertyDelegate);
    }
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("hayrack.progress", progress);
        nbt.putInt("hayrack.cookingTime", maxProgress);
    }
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        super.readNbt(nbt);
        progress = nbt.getInt("hayrack.progress");
        maxProgress = nbt.getInt("hayrack.cookingTime");
    }
    ...
    public static void tick(World world, BlockPos blockPos, BlockState state, HayRackBlockEntity entity) {
        if(world.isClient()) {
            return;
        }
    ...
    private static void craftItem(HayRackBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<HayRackRecipe> recipe = entity.getWorld().getRecipeManager()
                .getFirstMatch(HayRackRecipe.Type.INSTANCE, inventory, entity.getWorld());
    ...
    private static boolean hasRecipe(HayRackBlockEntity entity) {
        SimpleInventory inventory = new SimpleInventory(entity.size());
        for (int i = 0; i < entity.size(); i++) {
            inventory.setStack(i, entity.getStack(i));
        }

        Optional<HayRackRecipe> match = entity.getWorld().getRecipeManager()
                .getFirstMatch(HayRackRecipe.Type.INSTANCE, inventory, entity.getWorld());

        entity.maxProgress = entity.getWorld().getRecipeManager()
                .getFirstMatch(HayRackRecipe.Type.INSTANCE, inventory, entity.getWorld()).map(HayRackRecipe::getCookTime).orElse(20);
    ...
    public boolean canInsert(int slot, ItemStack stack, @Nullable Direction side) {
        Direction localDir = this.getWorld().getBlockState(pos).get(HayRackBlock.FACING);
    ...
    public boolean canExtract(int slot, ItemStack stack, Direction side) {
        Direction localDir = this.getWorld().getBlockState(this.pos).get(HayRackBlock.FACING);
